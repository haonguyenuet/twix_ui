        ...
        child: Align(
          ...
          child: () {
            return switch (variant) {
              SelectVariant.fruits => TwixSelect<String>(
                  minWidth: 350,
                  placeholder: const Text('Select a fruit'),
                  options: fruits.entries.map((e) => TwixOption(value: e.key, child: Text(e.value))),
                  selectedOptionBuilder: (context, value) => Text(fruits[value]!),
                  onChanged: print,
                ),
              SelectVariant.timezone => ConstrainedBox(
                  constraints: const BoxConstraints(minWidth: 280),
                  child: TwixSelect<String>(
                    ...
                    options: timezones.entries.map(
                      (zone) => Column(
                        ...
                        children: [
                          ...
                          ...zone.value.entries.map((e) => TwixOption(value: e.key, child: Text(e.value)))
                        ],
                      ),
                    ),
                    onChanged: print,
                    selectedOptionBuilder: (context, value) {
                      final timezone =
                          timezones.entries.firstWhere((element) => element.value.containsKey(value)).value[value];
...
class _SelectWithSearchState extends State<SelectWithSearch> {
  ...
  @override
  Widget build(BuildContext context) {
    return TwixSelect<String>.withSearch(
      minWidth: 180,
